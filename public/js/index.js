...
socket.on("serverOut", function(serverMsg) {
	...
	let html = '<li class="list-group-item text-primary"><b>' + serverMsg.name + '</b>:' + serverMsg.msg + '<i>(' + time + ')</i>' + '</li>'
...
socket.on("priMsg", function(priMsg) {
	...
	// 拼接输出的信息
	let time = (new Date()).toLocaleString()
	let html = '<li class="list-group-item text-primary"><b>' + priMsg.name + '</b>:' + priMsg.msg + '<i>(' + time + ')</i>' + '</li>'
...
socket.on("sysMsg", function(sysMsg) {
	...
	// 拼接输出的信息
	let time = (new Date()).toLocaleString()
	let html = '<li class="list-group-item text-primary sysMsg"><b>' + sysMsg.name + ':</b>' + sysMsg.msg + '<i>(' + time + ')</i>' + '</li>'
